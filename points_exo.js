...
let tooltip_template2 = document.createRange().createContextualFragment(
    `<div id="tooltip2" style="border-right: 6px solid #00257c; border-bottom: 6px solid #00257c; display: none; position: absolute; font-size: 13px; width: 280px; text-align: center; line-height: 1; padding: 6px; background: white; font-family: sans-serif;">
        ...
        <div id="discovery_tip2" style="text-aligndocument.addEventListener('mousedown', onMouseDown, false);
        document.addEventListener('touchstart', onMouseDown, false);
        ...
        function onMouseDown(event){
            ...
            let mouseVector = new THREE.Vector3(
                ...
                1);
            let mouse_position = [(event.clientX / window.innerWidth) * 2 - 1, -(event.clientY / window.innerHeight) * 2 + 1];
            my_checkIntersects(mouseVector, mouse_position);
        }
        ...
        function checkIntersects(mouse_vector, mouse_position) {
            ...
            if(multipleSelectionMode){
                ...
                if (intersects[0]) {
                    ...
                    // get all points data with the same name
                    let name = datum.pl_hostname;
                    let sameNameDatums = exoplanetData.filter(d => d.pl_hostname === name);
        
                    highlightPoint(datum, index);
                    showTooltip(mouse_position, sameNameDatums, index);
        ...
        function my_checkIntersects(mouse_vector, mouse_position) {
            raycaster.setFromCamera(mouse_vector, camera);
            let intersects = raycaster.intersectObject(points);
            ...
            if(multipleSelectionMode){
                ...
                if (intersects[0]) {
                    let sorted_intersects = sortIntersectsByDistanceToRay(intersects); // sort the intersected objects
                    let intersect = sorted_intersects[0]; // use the closest intersect
                    // get all intersect with same pl_hostname
                    let name = exoplanetData[intersect.index].hostname;
                    let sameNameIntersects = sorted_intersects.filter(intersect => exoplanetData[intersect.index].hostname === name);
                    // console.log(exoplanetData[intersect.index])
                    console.log(name, sameNameIntersects.length);
                    console.log(mouse_vector, mouse_position);
                    sameNameIntersects.forEach((intersect, i) => {
                        let index = intersect.index;
                        let datum = generated_points[index];
                        let datum_tt = exoplanetData[index];
                        highlightPoint(datum, index);
        ...
        let startip_template = document.createRange().createContextualFragment(
            <div id="startip" style="border-right: 0px solid #00257c; border-bottom: 0px solid #00257c; display: none; position: absolute; font-size: 13px; width: 280px; text-align: center; line-height: 1; padding: 6px; background: rgba(0, 0, 0, 0.5); color: white; font-family: sans-serif;">
                <div id="host_tip" style="text-decoration: underline; text-align: center; padding: 4px; margin-bottom: 4px;"></div>
                <div id="starCount_tip" style="text-align: left; padding: 4px; margin-bottom: 4px;"></div>
                <div id="planetCount_tip" style="text-align: left; padding: 4px; margin-bottom: 4px;"></div>
                <div id="distance_tip" style="text-align: left; padding: 4px; margin-bottom: 4px;"></div>
                <div id="inclination_tip" style="text-align: left; padding: 4px; margin-bottom: 4px;"></div>
                <div id="RA_tip" style="text-align: left; padding: 4px; margin-bottom: 4px;"></div>
                <div id="dec_tip" style="text-align: left; padding: 4px; margin-bottom: 4px;"></div>
            </div>`
            document.body.append(startip_template);
        
        
        
        // create tooltip html attributes
        let tooltip_template = document.createRange().createContextualFragment(
        `<div id="tooltip" style="border-right: 0px solid #00257c; border-bottom: 0px solid #00257c; display: none; position: absolute; font-size: 13px; width: 280px; text-align: center; line-height: 1; padding: 6px; background: rgba(0, 0, 0, 0.5); color: white; font-family: sans-serif;">
            <div id="name_tip" style="text-decoration: underline; text-align: center; padding: 4px; margin-bottom: 4px;"></div>
            <div id="discovery_tip" style="text-align: left; padding: 4px; margin-bottom: 4px;"></div>
            <div id="distance_tip" style="text-align: left; padding: 4px; margin-bottom: 4px;"></div>
            <div id="radius_tip" style="text-align: left; padding: 4px; margin-bottom: 4px;"></div>
        ...
        function show_startip(mouse_position, datum, index) {
            let tooltip_width = 120;
            let x_offset = -tooltip_width/2;
            let y_offset = 30;
            ...
            tooltip_state.left = mouse_position[0] + x_offset + 60;
            tooltip_state.top = mouse_position[1] + y_offset - 32;
            tooltip_state.name = datum.pl_hostname;
            tooltip_state.starCount = datum.starCount;
            tooltip_state.planetCount = datum.planetCount;
        ...
        function my_updateTooltip() {
            ...
            if(selectedDatums_tt.length > 2){
                $planetCount_tip.innerText = "Avg Number of Planets in System: " + tooltip_state.planetCount;
            }
            else{
                $planetCount_tip.innerText = "Number of Planets in System: " + tooltip_state.planetCount;
            }
        
            if(selectedDatums_tt.length > 2){
                $starCount_tip.innerText = "Avg Number of Stars in System: " + tooltip_state.starCount;
            }
            else{
                $starCount_tip.innerText = "Number of Stars in System: " + tooltip_state.starCount;
            }
        ...
        function showTooltip(mouse_position, datum, index) {
            if(selectedDatums_tt.length > 2){
                ...
                for(var i = 0; i < selectedDatums_tt.length; i++){
                    avgDist += selectedDatums_tt[i].sy_dist
                    avgRad += selectedDatums_tt[i].pl_radj;
                    avgTemp += selectedDatums_tt[i].st_teff;
                    avgInclination += selectedDatums_tt[i].pl_orbincl;
                    avgRA += selectedDatums_tt[i].ra;
                ...
                tooltip_state.name = datum.pl_name;
                tooltip_state.discovery = datum.discoverymethod;
                tooltip_state.distance = avgDist
                tooltip_state.radius = avgRad
                tooltip_state.temp = avgTemp
                tooltip_state.inclination = avgInclination
                ...
                tooltip_state.name = datum.document.addEventListener('mousedown', onMouseDown, false);
                document.addEventListener('touchstart', onMouseDown, false);
        ...
        function my_showTooltip(mouse_position, datum, index) {
        ...
        // create tooltip html attributes
        let tooltip_template2 = document.createRange().createContextualFragment(
            `<div id="tooltip2" style="border-right: 0px solid #00257c; border-bottom: 0px solid #000000; display: none; position: absolute; font-size: 13px; width: 280px; text-align: center; line-height: 1; padding: 0px; background: white; font-family: sans-serif;">
                <div id="name_tip2" style="text-decoration: underline; text-align: center; padding: 4px; margin-bottom: 4px;"></div>
                <div id="discovery_tip2" style="text-align: left; padding: 4px; margin-bottom: 4px;"></div>
                <div id="distance_tip2" style="text-align: left; padding: 4px; margin-bottom: 4px;"></div>
                <div id="radius_tip2" style="text-align: left; padding: 4px; margin-bottom: 4px;"></div>
        ...
        : left; padding: 4px; margin-bottom: 4px;"></div>
        <div id="distance_tip2" style="text-align: left; padding: 4px; margin-bottom: 4px;"></div>
        <div id="radius_tip2" style="text-align: left; padding: 4px; margin-bottom: 4px;"></div>
        <div id="temp_tip2" style="text-align: left; padding: 4px; margin-bottom: 4px;"></div>
        <div id="inclination_tip2" style="text-align: left; padding: 4px; margin-bottom: 4px;"></div>
